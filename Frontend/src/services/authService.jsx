...
export const getProviderEarnings = async (providerId) => {
  ...
  try {
    const response = await api.get(`${API_URL}/api/service/earnings/${providerId}`);
...
export const login = (userData) => {
  return api.post(`${API_URL}/api/auth/login`, userData);
};
...
export const refreshToken = async () => {
  ...
  try {
    const response = await api.post(`${API_URL}/api/auth/refresh`, null, {
  ...
  try {
    ...
    if (formData.get('serviceType') === 'mechanical') {
      ...
      if (images && images.length > 0) {
        ...
        return api.post(`${API_URL}/api/service/request`, formDataWithFiles, {
          headers: {
    ...
    return api.post(`${API_URL}/api/service/request`, payload);
  } catch (error) {
...
export const getUserServiceRequests = (userId) => {
  return api.get(`${API_URL}/api/service/requests/${userId}`);
};

export const getNearbyMechanics = (lat, lng, distance = 10) => {
  return api.get(`${API_URL}/api/mechanics/nearby?lat=${lat}&lng=${lng}&distance=${distance}`);
};

export const getNearbyDeliveryPersons = (lat, lng, distance = 10) => {
  return api.get(`${API_URL}/api/delivery/nearby?lat=${lat}&lng=${lng}&distance=${distance}`);
};

export const adminCreateMechanic = (data) => {
  return api.post(`${API_URL}/api/admin/create-mechanic`, data);
};

export const adminCreateDelivery = (data) => {
  return api.post(`${API_URL}/api/admin/create-delivery`, data);
};

export const acceptServiceRequest = (data) => {
  return api.post(`${API_URL}/api/service/accept`, data);
};

export const updateServiceStatus = (data) => {
  return api.post(`${API_URL}/api/service/status`, data);
};

export const payForService = (requestId, paymentInfo) => {
  return api.post(`${API_URL}/api/service/payment`, { requestId, paymentInfo });
};

export const createRazorpayOrder = (requestId) => {
  return api.post(`${API_URL}/api/service/payment/create-order`, { requestId });
};

export const verifyRazorpayPayment = (payload) => {
  return api.post(`${API_URL}/api/service/payment/verify`, payload);
};